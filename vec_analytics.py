...
def most_corr_vec(labs        : np.ndarray                 , 
    ...
    ws.shape = (1, 1, N)   # Expand ws for bradcasting.
    ...
    corr[ind, idx] = -np.inf    # Array slicing -- fill "diagonal" with -infinity 
                                # -- effectively eliminating themselves as their maximum correlate.

    # If exclude_labs is not None, set correlations will all these <labs> 
    # vectors to -infinity to exclude them from consideration.
    if eidx:
        for id in ind:
            corr[id, eidx] = -np.inf
...
def most_corr_vecs(labs        : np.ndarray                 , 
    ...
    # If exclude_labs is not None, set correlations will all these 
    # lab vectors to -infinity to exclude them from consideration.
    if eidx:
        for id in ind:
            corr[id, eidx] = -np.inf
